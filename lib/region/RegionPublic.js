...
var RegionPublic = /** @class */ (function (_super) {
    ...
    function RegionPublic() {
        ...
        _this.setBy = function (key, option) {
            ...
            var _a = _this, getResults = _a.private_getResults, private_actionTypes = _a.private_actionTypes;
        ...
        _this.getProps = function (key) {
            ...
            if (typeof key === 'string' || Array.isArray(key)) {
                return selectProps_1.selectProps(key, getLoading(key), getResults(key), getError(key));
            }
            return selectProps_1.selectProps(key.result || key.key, getLoading(key.loading || key.key), getResults(key.result || key.key), getError(key.error || key.key));
        };
        return _this;
    }
    return RegionPublic;

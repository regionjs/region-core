...
var RegionPublic = /** @class */ (function (_super) {
    ...
    function RegionPublic() {
        ...
        _this.setBy = function (key, option) {
            if (option === void 0) { option = {}; }
            var format = option.format, id = option.id;
            ...
            return function (result) {
                if (id !== undefined) {
                    // TODO TEST ME
                    var formattedResult_1 = formatResult_1.formatResultWithId({ result: result, snapshot: snapshot, format: format, id: id });
                    dispatch({ type: SET, payload: { key: key, results: formattedResult_1, id: id } });
                    return formattedResult_1[id];
                }
                var formattedResult = util_1.formatResult({ result: result, snapshot: snapshot, format: format });
        ...
        _this.loadBy = function (key, asyncFunction, option) {
            ...
            var _a = _this, private_store = _a.private_store, getResults = _a.private_getResults, private_actionTypes = _a.private_actionTypes, expiredTime = _a.expiredTime, getFetchTimes = _a.private_getFetchTimes, setBy = _a.setBy;
            var LOAD = private_actionTypes.LOAD, SET = private_actionTypes.SET;
            var dispatch = private_store.dispatch;
            var snapshot = getResults(key);
            var setKey = setBy(key, option);
            return function (params) { return __awaiter(_this, void 0, void 0, function () {
                var result, error_1;
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            if (util_1.shouldThrottle({ asyncFunction: asyncFunction, forceUpdate: forceUpdate, key: key, snapshot: snapshot, expiredTime: expiredTime, getFetchTimes: getFetchTimes })) {
                                util_1.deprecate('Snapshot inject is deprecated. If you do not want it load, you can simply not load it. You can get fetchTime in getProps method to control your load function.'); // tslint:disable max-line-length
        ...
        _this.getProps = function (key) {
            var _a = _this, private_getLoadings = _a.private_getLoadings, private_getResults = _a.private_getResults, private_getFetchTimes = _a.private_getFetchTimes, private_getErrors = _a.private_getErrors;
            var _b = util_1.formatKeys(key), keys = _b.keys, loadings = _b.loadings, results = _b.results, fetchTimes = _b.fetchTimes, errors = _b.errors;
        ...
        _this.unstable_effect = function (from, to, getDerivedStateFromProps) {
            var _a = _this, private_store = _a.private_store, private_actionTypes = _a.private_actionTypes, getProps = _a.getProps, private_getResults = _a.private_getResults, load = _a.load;
            ...
            var handleSubscribe = function () {
                ...
                if (prevLoading === false && loading === true) {
                    ...
                    try {
                        ...
                        if (!util_1.isAsync(result)) {
                            dispatch({ type: SET, payload: { key: to, result: result } });
                        }
                    }
                    catch (error) {
                        dispatch({ type: SET, payload: { key: to, result: undefined, error: error } });
                    }
                    // NOTE LOAD after SET

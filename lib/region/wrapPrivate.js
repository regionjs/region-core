...
var _getProps = _interopRequireDefault(require("../util/getProps"));
      ...
      _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "private_selectorFactory", function (key) {
        ...
        return function (state, ownProps) {
          if (typeof key === 'string' || Array.isArray(key)) {
            return (0, _getProps.default)(key, getLoading(key), getResults(key), getError(key));
          }
          ...
          var props = (0, _getProps.default)(key.result || key.key || key.entity, getLoading(key.loading || key.key || key.entity), getResults(key.result || key.key || key.entity), getError(key.error || key.key || key.entity));

...
var setKey = function (state, _a, cache) {
    var key = _a.key, result = _a.result, id = _a.id, error = _a.error;
    if (cache) {
        reducerPrototype_1.setValueDeep(state, [key, 'results', id], result);
...
exports.createStore = function () {
    ...
    var load = function (payload) {
        ...
        // I will decide later
        var simpleState = state[key] || {};
        if (simpleState.results) {
            var result = simpleState.results[id];
            reducerPrototype_1.setValueDeep(state, [key, 'result'], result);
        }
        reducerPrototype_1.setValueDeep(state, [key, 'promise'], promise);
    ...
    var set = function (payload, cache) {
        ...
        var nextState = setKey(state, { key: key, result: result, id: id, error: error }, cache);
        if (error) {
